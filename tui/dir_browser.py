...
class DirectoryBrowserScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            ...
            yield Button(
                "Seleccionar carpeta actual", id="select-here", variant="success"
            )
        yield Footer()
    ...
    def _fill(self) -> None:
        ...
        for index, p in enumerate(core.music_dir_candidates()):
            ...
            shortcuts.append(ListItem(Label(f"📁 {p}"), id=item_id))
        ...
        for index, sub in enumerate(core.list_subdirs(self.current)):
            ...
            dirs.append(ListItem(Label(f"📁 {sub.name}"), id=item_id))
