...
def main():
    ...
    parser.add_argument(
        "--lrc-dir", type=Path, required=True, help="Directory containing LRC files"
    )
    parser.add_argument(
        "--audio-dir", type=Path, help="Directory containing audio files"
    )
    parser.add_argument(
        "--wlrc", action="store_true", help="LRC files are word-level (WLRC format)"
    )
    parser.add_argument(
        "--font", type=str, default="block", help="Font to use (default: block)"
    )
    parser.add_argument(
        "--custom-fonts", type=Path, help="Path to custom fonts JSON file"
    )
    parser.add_argument(
        "--refresh-rate",
        type=float,
    ...
    if args.custom_fonts:
        if not args.custom_fonts.exists():
            print(
                f"Error: custom fonts file {args.custom_fonts} does not exist",
                file=sys.stderr,
            )
            return 1
    ...
    if play_audio and args.audio_dir:
        ...
        print(
            "Aviso: --play requiere --audio-dir para encontrar el archivo",
            file=sys.stderr,
        )
        play_audio = False
