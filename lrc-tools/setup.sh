...
check_cmd mpv "sudo pacman -S mpv   # reproducción en el visualizador"
...
if [[ -d "$SCRIPT_DIR/lrc_tools" ]]; then
    ...
    if [[ -d "$SCRIPT_DIR/tui" ]]; then
        mkdir -p "$PKG_DIR/tui"
        cp "$SCRIPT_DIR/tui/"*.py "$PKG_DIR/tui/"
    fi
    # core + tui_main for TUI
    for f in core.py tui_main.py; do
        [[ -f "$SCRIPT_DIR/$f" ]] && cp "$SCRIPT_DIR/$f" "$PKG_DIR/"
    done
fi
...
write_stub lrc-tools      tui_main

# short alias: lt -> lrc-tools
ln -sf "$BIN_DIR/lrc-tools" "$BIN_DIR/lt"
echo "  ✓ lt → $BIN_DIR/lt (symlink)"
...
echo "                --wlrc"
echo
echo "  lrc-tools    # TUI interactiva (o: lt)"
