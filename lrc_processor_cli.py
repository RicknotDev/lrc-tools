...
def main():
    ...
    parser.add_argument(
        "--wlrc", action="store_true", help="Convert output to word-level WLRC format"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-words", type=int, default=None, help="Max words per phrase (default: 8)"
    )
    parser.add_argument(
        "--no-split-commas", action="store_true", help="Do not split phrases at commas"
    )
    parser.add_argument(
        "--onset-detection",
        action="store_true",
    ...
    cfg = Config(args.config).processor if Config else None

    max_phrase_duration = args.max_phrase_duration or (
        cfg.max_phrase_duration if cfg else 2.5
    )
    min_phrase_duration = args.min_phrase_duration or (
        cfg.min_phrase_duration if cfg else 0.3
    )
    max_words_per_phrase = args.max_words or (cfg.max_words_per_phrase if cfg else 8)
    split_on_commas = not args.no_split_commas
    use_onset_detection = args.onset_detection or (
        cfg.use_onset_detection if cfg else False
    )
    ...
    print(
        f"Output format: {'WLRC (word-level)' if args.wlrc else 'LRC (phrase-level)'}"
    )
