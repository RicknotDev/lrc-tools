...
from lrc_tools.splitter import split_phrase_intelligently


def detect_onset_positions(
    audio_path: Path, start_time: float, end_time: float, num_words: int,
    blend_factor: float = 0.5,
) -> List[float]:
    try:
        librosa = check_optional(
            "librosa",
            'python3 -m pip install --user "lrc-tools[timing]"',
            "word-level onset detection",
        )
        if librosa is None:
        ...
        y, sr = librosa.load(
            str(audio_path), sr=22050, offset=start_time, duration=phrase_duration
        )
        ...
        normalized_onsets = [
            max(0.0, min(1.0, pos / phrase_duration)) for pos in onset_positions
        ]

        even_distribution = [i / num_words for i in range(num_words)]

        return [
            even_distribution[i] * (1 - blend_factor)
            + normalized_onsets[i] * blend_factor
            for i in range(num_words)
        ]

    except Exception as e:
        print(f"[Warning] Onset detection unavailable or failed: {e}")
        return [i / num_words for i in range(num_words)]


def phrases_to_words(
    phrases: List[Dict], audio_path: Optional[Path] = None,
    blend_factor: float = 0.5,
) -> List[Dict]:
    words = []
    ...
    for i, phrase in enumerate(phrases):
        ...
        if audio_path and audio_path.exists():
            onset_positions = detect_onset_positions(
                audio_path, phrase_start, phrase_end, len(phrase_words),
...
def process_long_phrases(
    ...
    for i, line in enumerate(lines):
        ...
        should_split = has_commas or (
            actual_duration > max_phrase_duration
            and len(line["text"].split()) > max_words_per_phrase
        )

        if should_split:
            result.extend(
                split_phrase_intelligently(
                    line["text"],
                    actual_duration,
                    line["timestamp"],
                    max_phrase_duration,
...
def process_lrc_file(
    ...
    from lrc_tools.audio import find_audio_for_lrc, get_audio_duration
    from lrc_tools.parser import parse_lrc, write_lrc
    ...
    if output_wlrc:
        processed = phrases_to_words(processed, audio_path, blend_factor=onset_blend_factor)
        if verbose:
            print(f"[Words] Converted to {len(processed)} word-level lines")

    header_comments = [
        "Processed by lrc-tools",
